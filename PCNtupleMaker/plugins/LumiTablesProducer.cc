...
class LumiTablesProducer : public edm::global::EDProducer<> {
public:
  LumiTablesProducer(edm::ParameterSet const& params){
  ...
  ~LumiTablesProducer() override {}

  void produce(edm::StreamID id, edm::Event& iEvent, const edm::EventSetup& iSetup) const override {
    auto npuTab = std::make_unique<nanoaod::FlatTable>(1, "Pileup", true);//, true);
 ...
 // void fillNPUObjectTable(const std::vector<PileupSummaryInfo>& npuProd, nanoaod::FlatTable& out, double refpvz) const {
   void fillNPUObjectTable(nanoaod::FlatTable& out) const{   
 // Get BX 0
 //   unsigned int bx0 = 0;
    unsigned int nt = 0;
  //  unsigned int npu = 0;
...
*/
    ...
    out.addColumnValue<int>("nTrueInt",
                              nt,
...
DEFINE_FWK_MODULE(LumiTablesProducer);
